	...
	<div id="myCarousel" class="carousel slide">
		<div class="carousel-inner">
			...
			<div class="item @if($key==0) active @endif">
				<div class="container">
					<a @if(!empty($banner['link'])) href="{{ url($banner['link']) }}" @else href="javascript:void(0)" @endif><img style="width:100%" src="{{ asset('dashboard/dist/img/banners_img/'.$banner['image']) }}" alt="{{ ($banner['alt']) }}" title="{{ ($banner['title']) }}" /></a>
				</div>
			</div>
			@endforeach
		</div>
